...
function enviarAGoogleSheets() {
    ...
    // Campos especiales de Pluma (campos base, columnas simples)
    const jn = document.getElementById('joystick_n')?.value || '';
    const jEstado = document.getElementById('joystick_n_b')?.checked ? 'Bueno' : document.getElementById('joystick_n_m')?.checked ? 'Malo' : '';
    datos.controlJoystick = (jn ? 'N° ' + jn : '') + (jn && jEstado ? ' - ' : '') + jEstado;
    datos.bateria1 = document.getElementById('bateria1_b')?.checked ? 'Bueno' : document.getElementById('bateria1_m')?.checked ? 'Malo' : '';
    datos.bateria2 = document.getElementById('bateria2_b')?.checked ? 'Bueno' : document.getElementById('bateria2_m')?.checked ? 'Malo' : '';
