...
function setupCanvas(img, canvas, ctx, id) {
    let isDrawing = false;
...
function generatePDF() {
    ...
    doc.text('CHECK LIST FURGON', margin + col1Width + col2Width/2, yPos + 7, { align: 'center' });
    doc.text('CARROZADOS', margin + col1Width + col2Width/2, yPos + 12, { align: 'center' });
    
    doc.rect(margin + col1Width + col2Width, yPos, col3Width, headerHeight);
    doc.setFontSize(8);
    doc.text('Registro: CHECK-MOVIL-0001', margin + col1Width + col2Width + 2, yPos + 3.5);
    yPos += headerHeight;
    ...
    doc.save(`CHECK-MOVIL-0001-${patente || 'FURGON'}-${fecha}.pdf`);
